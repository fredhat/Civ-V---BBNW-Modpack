...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		amounts_of_resources_placed = table.fill(0, MAX_RESOURCE_INDEX), -- Stores amounts of each resource ID placed. WARNING: This table uses adjusted resource ID (+1) to account for Lua indexing. Add 1 to all IDs to index this table.	-- MOD.Barathor: Updated -- original = 45 -- any new luxuries added post BNW start at ID 40, so this needs to be increased!
		...
		cedar_ID, feathers_ID, ginger_ID, lotus_ID, oak_ID, obsidian_ID, opium_ID, rubber_ID, saffron_ID, sulphur_ID, vanilla_ID,
...
function AssignStartingPlots:__Init()
	...
	for resource_data in GameInfo.Resources() do
		...
		-- Exploration Resources IDs
		elseif resourceType == "RESOURCE_JFD_CEDAR" then
			self.cedar_ID = resourceID;
		elseif resourceType == "RESOURCE_JFD_FEATHERS" then
			self.feathers_ID = resourceID;
		elseif resourceType == "RESOURCE_JFD_GINGER" then
			self.ginger_ID = resourceID;
		elseif resourceType == "RESOURCE_JFD_LOTUS" then
			self.lotus_ID = resourceID;
		elseif resourceType == "RESOURCE_JFD_OAK" then
			self.oak_ID = resourceID;
		elseif resourceType == "RESOURCE_JFD_OBSIDIAN" then
			self.obsidian_ID = resourceID;
		elseif resourceType == "RESOURCE_JFD_OPIUM" then
			self.opium_ID = resourceID;
		elseif resourceType == "RESOURCE_JFD_RUBBER" then
			self.rubber_ID = resourceID;
		elseif resourceType == "RESOURCE_JFD_SAFFRON" then
			self.saffron_ID = resourceID;
		elseif resourceType == "RESOURCE_JFD_SULFUR" then
			self.sulphur_ID = resourceID;
		elseif resourceType == "RESOURCE_JFD_VANILLA" then
			self.vanilla_ID = resourceID;
		end
	end
end
------------------------------------------------------------------------------
	...
	elseif plotType == PlotTypes.PLOT_OCEAN then
		if terrainType == TerrainTypes.TERRAIN_COAST and featureType == FeatureTypes.NO_FEATURE then
			if plot:IsLake() == false then -- Place Fish
				...
				plot:SetResourceType(self.lotus_ID, 1);
				--print("Placed Lotus.");
				self.amounts_of_resources_placed[self.lotus_ID + 1] = self.amounts_of_resources_placed[self.lotus_ID + 1] + 1;
				return true, false
			end
		end
	end	
...
function AssignStartingPlots:PlaceNaturalWonders()
	...
	local worldsizes = {
		...
		[GameInfo.Worlds.WORLDSIZE_STANDARD.ID] = 10,
		[GameInfo.Worlds.WORLDSIZE_LARGE.ID] = 12,
		[GameInfo.Worlds.WORLDSIZE_HUGE.ID] = 14
		}
...
function AssignStartingPlots:GenerateGlobalResourcePlotLists()
	...
	self.lakes = GetShuffledCopyOfTable(temp_lakes)
	--
					...
					if plot:GetResourceType(-1) == -1 then	
						...
						elseif plotType == PlotTypes.PLOT_HILLS or plotType == PlotTypes.PLOT_LAND then
							...
							elseif terrainType == TerrainTypes.TERRAIN_DESERT then
								...
								--
								allowed_luxuries[self.opium_ID] = true
							elseif terrainType == TerrainTypes.TERRAIN_PLAINS then
								allowed_luxuries[self.marble_ID] = true
								allowed_luxuries[self.gold_ID] = true
								allowed_luxuries[self.silver_ID] = true
								...
								allowed_luxuries[self.incense_ID] = true
								--
								allowed_luxuries[self.ginger_ID] = true
								allowed_luxuries[self.obsidian_ID] = true
								allowed_luxuries[self.saffron_ID] = true
								allowed_luxuries[self.vanilla_ID] = true
							elseif terrainType == TerrainTypes.TERRAIN_GRASS then
								allowed_luxuries[self.marble_ID] = true
								allowed_luxuries[self.gold_ID] = true
								allowed_luxuries[self.silver_ID] = true
							...
							if plotType == PlotTypes.PLOT_OCEAN then
								if terrainType == TerrainTypes.TERRAIN_COAST then
									if plot:IsLake() == false then
										...
										table.insert(region_lakes, plotIndex);
									end
								end
							elseif plotType == PlotTypes.PLOT_HILLS and terrainType ~= TerrainTypes.TERRAIN_SNOW then
								if featureType == FeatureTypes.NO_FEATURE then
...
function AssignStartingPlots:GetIndicesForLuxuryType(resource_ID)
	...
	elseif resource_ID == self.obsidian_ID then
		primary, secondary, tertiary, quaternary, quinary, senary = 8, 15, 28, 37, 24, 2;
	elseif resource_ID == self.vanilla_ID then
		primary, secondary, tertiary, quaternary, quinary, senary = 8, 15, 28, 37, 24, 2;
	-- Land Luxuries
	...
	elseif resource_ID == self.cotton_ID then
		primary, secondary, tertiary, quaternary, quinary, senary = 16, 11, 33, 24, 28, 2;
	elseif resource_ID == self.ginger_ID then
		primary, secondary, tertiary, quaternary, quinary, senary = 16, 11, 33, 24, 28, 2;
	elseif resource_ID == self.opium_ID then
		primary, secondary, tertiary, quaternary, quinary, senary = 16, 11, 33, 24, 28, 2;
	elseif resource_ID == self.saffron_ID then
		primary, secondary, tertiary, quaternary, quinary, senary = 16, 11, 33, 24, 28, 2;
	end
...
function AssignStartingPlots:PlaceSexyBonusAtCivStarts()
	...
	for region_number = 1, self.iNumCivs do
		...
		local plot_list, fish_list, lotus_list = {}, {}, {};
		-- For notes on how the hex-iteration works, refer to PlaceResourceImpact()
					...
					if plot:GetResourceType(-1) == -1 and featureType ~= FeatureTypes.FEATURE_OASIS then -- No resource or Oasis here, safe to proceed.
						...
						if plotType == PlotTypes.PLOT_OCEAN then
							if not plot:IsLake() then
								...
								table.insert(lotus_list, plotIndex);
							end
						end
					end
				end
		...
		if iNumCandidates > 0 then
			...
			if iFishCandidates > 0 then
				...
				local iLotusCandidates = table.maxn(lotus_list);
				if iLotusCandidates > 0 then
					print("Placing 'sexy Lotus' in third ring of start location in Region#", region_number);
					local shuf_list = GetShuffledCopyOfTable(lotus_list)
					local iNumLeftToPlace = self:PlaceSpecificNumberOfResources(self.lotus_ID, 1, 1, 1, -1, 0, 0, shuf_list);
				end
			end
		end
	end
	...
	for y = 0, iH - 1 do
		for x = 0, iW - 1 do
			...
			elseif res_ID == self.cocoa_ID or 
				   ...
				   res_ID == self.vanilla_ID or 
				   res_ID == self.cedar_ID or 
				   res_ID == self.oak_ID or 
				   res_ID == self.rubber_ID or 
				   res_ID == self.deer_ID then
				
				if res_ID == self.fur_ID or res_ID == self.feathers_ID or res_ID == self.vanilla_ID or res_ID == self.cedar_ID or res_ID == self.oak_ID or res_ID == self.rubber_ID then
				...
				if (featureType == FeatureTypes.FEATURE_MARSH) then
					if res_ID == self.sugar_ID or res_ID == self.spices_ID or res_ID == self.dye_ID then
						...
						if lat <= AvgJungleRange then
							if res_ID ~= self.deer_ID and res_ID ~= self.fur_ID and res_ID ~= self.cedar_ID and res_ID ~= self.oak_ID then
			...
			elseif res_ID == self.incense_ID or 
				   ...
				   res_ID == self.ginger_ID or 
				   res_ID == self.opium_ID or 
				   res_ID == self.saffron_ID or 
				   res_ID == self.sulphur_ID or 
				   res_ID == self.cotton_ID then 
				
				if res_ID == self.ivory_ID or res_ID == self.ginger_ID or res_ID == self.opium_ID or res_ID == self.saffron_ID then
...
function AssignStartingPlots:PrintFinalResourceTotalsToLog()
	...
	print("- Exploration LUXURY Resources -")
	print(self.ginger_ID,      "Ginger..: ", self.amounts_of_resources_placed[self.ginger_ID + 1])
	print(self.obsidian_ID,  "Obsidian: ", self.amounts_of_resources_placed[self.obsidian_ID + 1])
	print(self.opium_ID,    "Opium...: ", self.amounts_of_resources_placed[self.opium_ID + 1])
	print(self.saffron_ID,     "Saffron.: ", self.amounts_of_resources_placed[self.saffron_ID + 1])
	print(self.vanilla_ID,   "Vanilla.: ", self.amounts_of_resources_placed[self.vanilla_ID + 1])
	print("-")
	...
	print("- Exploration BONUS Resources -")
	print(self.cedar_ID,   "Cedar...: ", self.amounts_of_resources_placed[self.cedar_ID + 1])
	print(self.feathers_ID,   "Feathers: ", self.amounts_of_resources_placed[self.feathers_ID + 1])
	print(self.lotus_ID,      "Lotus...: ", self.amounts_of_resources_placed[self.lotus_ID + 1])
	print(self.oak_ID,  "Oak.....: ", self.amounts_of_resources_placed[self.oak_ID + 1])
	print(self.rubber_ID,    "Rubber..: ", self.amounts_of_resources_placed[self.rubber_ID + 1])
	print(self.sulphur_ID,     "Sulphur.: ", self.amounts_of_resources_placed[self.sulphur_ID + 1])
	print("-");
	print("-----------------------------------------------------");
end
------------------------------------------------------------------------------
...
function AssignStartingPlots:PlaceStrategicAndBonusResources()
	...
	local resources_to_place = {
	{self.cedar_ID, 1, 100, 1, 3} };
	self:ProcessResourceList(20 * bonus_multiplier, 3, self.forest_flat_list, resources_to_place)
	...
	local resources_to_place = {
	{self.feathers_ID, 1, 100, 2, 3} };
	self:ProcessResourceList(15 * bonus_multiplier, 3, self.jungle_flat_list, resources_to_place)
	
	local resources_to_place = {
	{self.rubber_ID, 1, 100, 2, 3} };
	self:ProcessResourceList(15 * bonus_multiplier, 3, self.jungle_flat_list, resources_to_place)

	local resources_to_place = {
	{self.sulphur_ID, 1, 100, 2, 3} };
	self:ProcessResourceList(15 * bonus_multiplier, 3, self.hills_open_no_desert, resources_to_place)
	
	local resources_to_place = {
	{self.lotus_ID, 1, 100, 2, 3} };
	self:ProcessResourceList(15 * bonus_multiplier, 3, self.lakes, resources_to_place)
end
------------------------------------------------------------------------------
