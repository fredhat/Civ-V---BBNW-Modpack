...
function UpdateUnitPromotions(pUnit)
	...
	if pUnit then
		...
		for unitPromotion in GameInfo.UnitPromotions() do
			...
			if unitPromotion.ShowInUnitPanel == 1 and pUnit:IsHasPromotion(unitPromotionID) then
...
function UpdateCombatOddsUnitVsCity(pMyUnit, pCity)
	...
	if pMyUnit ~= nil then
		...
		-- Ranged Unit
		local isRanged = pMyUnit:GetBaseRangedCombatStrength() > 0 and (pMyUnit:IsRangedSupportFire() == false)
		if isRanged then
		...
		if iMyStrength > 0 then
			...
			-- Unit's max HP
			local iMyMaxHitPoints = pMyUnit:GetMaxHitPoints()
			if iTheirDamageInflicted > iMyMaxHitPoints then
				iTheirDamageInflicted = iMyMaxHitPoints
			end
			...
			if pMyUnit:GetDamage() + iTheirDamageInflicted >= iMyMaxHitPoints then
				bMyUnitLoss = true
			end
			-- now do the health bars
			DoUpdateHealthBars(iMyMaxHitPoints, maxCityHitPoints, pMyUnit:GetDamage(), pCity:GetDamage(), iMyDamageInflicted, iTheirDamageInflicted)
			-- Now do text stuff
			local controlTable
			local strText
			...
			-- Our unit is weak
			elseif (pMyUnit:GetDamage() > (iMyMaxHitPoints / 2)) and iTheirDamageInflicted > 0 then
...
function UpdateCombatOddsUnitVsUnit(pMyUnit, pTheirUnit)
	...
	if pMyUnit ~= nil then
		...
		if iMyStrength > 0 then
			local iMyMaxHitPoints = pMyUnit:GetMaxHitPoints()
			local iTheirMaxHitPoints = pTheirUnit:GetMaxHitPoints()
			-- Start with logic of combat estimation
			local iMyDamageInflicted = 0
			local iTheirDamageInflicted = 0
			local iTheirFireSupportCombatDamage = 0
			...
			if iMyDamageInflicted > iTheirMaxHitPoints then
				iMyDamageInflicted = iTheirMaxHitPoints
			end
			if iTheirDamageInflicted > iMyMaxHitPoints then
				iTheirDamageInflicted = iMyMaxHitPoints
			end
			-- now do the health bars
			DoUpdateHealthBars(iMyMaxHitPoints, iTheirMaxHitPoints, pMyUnit:GetDamage(), pTheirUnit:GetDamage(), iMyDamageInflicted, iTheirDamageInflicted)
			-- Now do text stuff
			local controlTable
			local strText
			--local strMyDamageTextColor = "White_Black";
			...
			--if (bRanged) then
				--strText = ".";
			--else
				--strText = "DAMAGE Taken: ~" .. iTheirDamageInflicted .. "/" .. iTheirMaxHitPoints;
			...
			--controlTable = g_TheirCombatDataIM:GetInstance();
			--strText = "DAMAGE Taken: ~" .. iMyDamageInflicted .. "/" .. iMyMaxHitPoints;
			Controls.RangedAttackIndicator:SetHide(true)
			Controls.SafeAttackIndicator:SetHide(true)
			Controls.RiskyAttackIndicator:SetHide(true)
			Controls.TotalVictoryIndicator:SetHide(true)
							...
							for i = 0, iNumUnits do
								...
								if pUnit ~= nil and not pUnit:IsInvisible(iTeam, false) then
									...
									--if (pUnit:GetDomainType() ~= DomainTypes.DOMAIN_AIR) then
										...
										if pHeadUnit:GetDomainType() == pUnit:GetDomainType() or pHeadUnit:IsRanged() or (pHeadUnit:GetDomainType() == DomainTypes.DOMAIN_SEA and (pUnit:IsEmbarked() or pUnit:IsHoveringUnit())) then
