-- GlobalArchaeologistDigSites
-- Original Author: William Howard
--------------------------------------------------------------
local iArchaeologist = GameInfoTypes.UNIT_ARCHAEOLOGIST
local iAestheticsFinisher = GameInfoTypes.POLICY_AESTHETICS_FINISHER
...
function OnUnitSelectionChange(iPlayer, iUnit, iHexX, iHexY, _, bSelected)
	if bSelected then
		...
		if pUnit then
			if pUnit:GetUnitType() == iArchaeologist then
				ShowDigSites(pPlayer, pPlayer:HasPolicy(iAestheticsFinisher))
			end
		end
	else
		ClearDigSites()
